...
//    		var_dump($_POST);die;
    	...
    	// 取出当前商品的属性数据
    	$gaModel = M('GoodsAttr');
    	$gaData = $gaModel->field('a.*,b.attr_name,b.attr_type,b.attr_option_values')->alias('a')->join('LEFT JOIN jd_attribute b ON a.attr_id=b.id')->where(array('a.goods_id'=>array('eq', $id)))->order('a.attr_id ASC')->select();
    ...
    public function goods_number()
    {
    	...
    	$sql = 'SELECT a.*,b.attr_name FROM jd_goods_attr a 
    	  LEFT JOIN jd_attribute b ON a.attr_id=b.id
    	  WHERE attr_id IN(
    		SELECT attr_id FROM jd_goods_attr WHERE goods_id='.$goodsId.' GROUP BY attr_id HAVING count(*) > 1
