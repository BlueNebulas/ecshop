...
class GoodsModel extends Model 
{
	...
	public function search($pageSize = 20, $isDelete = 0)
	{
		...
		$data['page'] = $page->show();
		/************************************** 取数据 ******************************************/
		$data['data'] = $this->field('a.*,IFNULL(SUM(b.goods_number),0) gn')->alias('a')->join('LEFT JOIN jd_goods_number b ON a.id=b.goods_id')->where($where)->group('a.id')->order('id DESC')->limit($page->firstRow.','.$page->listRows)->select();
		return $data;
	}
	// 添加前
	...
	public function convertGoodsAttrIdToGoodsAttrStr($gaid)
	{
		if($gaid)
		{
			$sql = 'SELECT GROUP_CONCAT( CONCAT( b.attr_name,  ":", a.attr_value ) SEPARATOR  "<br />" ) gastr FROM jd_goods_attr a LEFT JOIN jd_attribute b ON a.attr_id = b.id WHERE a.id IN ('.$gaid.')';
			$ret = $this->query($sql);
			return $ret[0]['gastr'];
		}
		else 
	...
	public function search_goods()
	{
		...
		/*********************** 取商品 **********************/
		$data['data'] = $this->field('a.id,a.goods_name,sm_logo,shop_price,IFNULL(SUM(b.goods_number),0) xl,(SELECT count(id) FROM jd_comment c WHERE c.goods_id=a.id) pl')->alias('a')->join('LEFT JOIN jd_order_goods b ON (a.id=b.goods_id AND b.order_id IN(SELECT id FROM jd_order WHERE pay_status=1))')->where($where)->group('a.id')->order("$orderBy $orderWay")->limit($page->firstRow.','.$page->listRows)->select();
		return $data;
	}
}
